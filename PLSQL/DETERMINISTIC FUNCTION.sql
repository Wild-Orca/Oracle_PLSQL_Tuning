Deterministic Functions for PL/SQL Optimization
    In Oracle Database, declaring a function as DETERMINISTIC can help optimize PL/SQL performance by allowing the database to reuse function results
    , avoiding unnecessary recomputation for the same inputs. A deterministic function always returns the same result when given the same arguments, which makes it eligible for caching and optimization.
...
By marking this function as DETERMINISTIC, the database understands that for the same p_customer_id, the function will always return the same discount rate. 
This allows Oracle to cache the result and reuse it, rather than running the function each time it's called.
